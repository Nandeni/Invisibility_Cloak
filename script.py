...
def create_background(cap, num_frames = 30):
    ...
    for i in range(num_frames):
        ret, frames = cap.read()
        if ret:
            backgrounds.append(frames)
        else:
            print(f"Warning: Could not frame {i+1}/{num_frames}")
        time.sleep(0.1)
    if backgrounds:
        return np.median(backgrounds, axis=0).astype(np.uint8)
    else:
        raise ValueError("Could not capture any fraames for background")
...
def main():
    ...
    lower_white = np.array([5, 100, 100])
    ...
    while True:
        ...
        mask = create_cloak(frame, lower_white, upper_white)
